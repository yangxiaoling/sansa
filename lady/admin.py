...
class UserProfileAdmin(BaseUserAdmin):
    ...
    list_filter = ()  # 没有的话, 会继承父类的list_filter, 因没有is_staff字段而报错
    fieldsets = (  # admin界面中分块显示信息
        (None, {'fields': ('email', 'password')}),
        ('Personal info', {'fields': ('name',)}),
        ('Permissions', {'fields': ('is_superuser', 'is_active', 'bind_hosts', 'host_groups', 'user_permissions', 'groups')}),  # 这里也要加, 负责显示
    )
    ...
    search_fields = ('email',)
    ordering = ('email',)
    # filter_horizontal = ()  # 不设置时为空, 否则会继承, 变成如下效果:
    filter_horizontal = ('user_permissions', 'groups', 'bind_hosts', 'host_groups')  # PermissionsMixin中定义的字段
